...
-- record_set_xp is callable individually (concurrent-guard regression test
-- exercises the (user_id, set_id) UNIQUE INDEX through this entry point) and
-- is preserved for one-set diagnostic use. The hot path — save_workout — no
-- longer calls it in a loop; that path runs through record_session_xp_batch
...
-- ---------------------------------------------------------------------------
-- 11b. record_session_xp_batch(p_workout_id) — single-pass session XP roll-up
-- ---------------------------------------------------------------------------
--
-- BUG-RPG-004 fix. The original save_workout did
--     FOR v_set_id IN (SELECT id FROM sets ...) LOOP
--       PERFORM record_set_xp(v_set_id);
--     END LOOP
-- which paid full PL/pgSQL function-dispatch + parameter-binding +
-- session/weekly-volume subquery cost per set. EXPLAIN ANALYZE on a 100-set
-- payload showed ~424 ms (8.5× over the 50 ms gate); the per-call cost was
-- ~4 ms warm × 100 sets ≈ 400 ms of pure dispatch overhead, with the actual
-- math costing < 1 ms.
--
-- The batch is a SINGLE PL/pgSQL invocation that:
--
--   1. Loads all eligible sets in (workout_exercise.order, set.set_number)
--      order via one JOIN — same total ordering record_set_xp's loop used.
--   2. Pre-fetches the prior peak_weight for every distinct exercise in this
--      session — one query, hashed into a jsonb map (small, O(distinct
--      exercises ≤ ~25), peak advancement is rare → not the hot path).
--   3. Pre-computes the prior weekly volume per body_part from xp_events
--      OUTSIDE this session in the past 7 days — one query, materialized
--      into a fixed-size numeric[] indexed by the BodyPart enum (1..7).
--      Within-session contributions accumulate during the loop with O(1)
--      array writes (NOT jsonb concat — see perf note below).
--   4. Iterates sets in memory, advancing peak BEFORE strength (matches
--      record_set_xp), accumulating session_vol and weekly_vol per body
--      part using fixed-size arrays. Per-bp xp totals also accumulate into
--      a fixed-size array for the body_part_progress UPSERT.
--   5. Bulk-INSERTs all xp_events rows (UNNEST arrays).
--   6. Aggregates per-bp totals and UPSERTs body_part_progress in one
--      statement; rank is recomputed via rpg_rank_for_xp(new_total).
--   7. Aggregates per-exercise peak advances and UPSERTs exercise_peak_loads
--      in one statement.
--
-- PERF NOTE — fixed-size float8[] arrays vs jsonb maps + numeric math:
--   The first cut of this batch used `jsonb_build_object || running_map`
--   to advance per-bp running session_vol / weekly_vol / bp_total state.
--   Each `||` allocates and fully copies the LHS jsonb — O(N) in jsonb
--   size — so ~3 body-parts × 100 sets × 4 maps ≈ 1200 jsonb concat ops
--   on growing maps compounded to ~270 ms warm.
--
--   Replacing the running maps with arrays indexed 1..7 by the BodyPart
--   enum (fixed cardinality) made the accumulator updates O(1), but the
--   loop was still ~250 ms — CPU-bound, not I/O. Profiling identified
--   PostgreSQL's `numeric` (arbitrary-precision software math) as the
--   true bottleneck: 100 sets × 3 bps × ~6 numeric multiplications =
--   ~1800 numeric ops with progressively-growing precision digits.
--
--   The fix: run the inner-loop math in `float8` (IEEE 754 double,
--   hardware-accelerated). Only round to numeric(14,4) at the storage
--   boundary — when emitting xp_events.attribution, .payload, .total_xp,
--   and body_part_progress.total_xp. The Phase 18a spec parity tolerance
--   is 0.0001 (matching the storage column's scale of 4); float8's ~15
--   significant decimal digits is far more than needed. End-to-end
--   speedup: ~25× (265 ms → ~11 ms warm in EXPLAIN ANALYZE; ~40 ms p95
--   on the Dart-end perf bench, well under the 50 ms gate).
--
--   Helpers (rpg_base_xp / rpg_intensity_for_reps / rpg_strength_mult)
--   keep their numeric signatures because they're shared with
--   record_set_xp; they're called once per set, not per body-part, so
--   their numeric cost is negligible. We cast to float8 once after the
--   helper call and stay in float8 through the inner loop.
--
-- Numeric parity with the per-set path is preserved within the spec's
-- 0.0001 tolerance: the math is the same, the iteration order is the
-- same, and per-bp XP is rounded to numeric(14,4) at the same boundary
-- the per-set path applied (the storage column). The 16 integration
-- tests in test/integration/ are the regression gate.
--
-- This function is only called from save_workout (which has already
-- validated auth and run the reversal pattern). It assumes the caller has
-- already deleted prior xp_events for this session via cascade.
--
-- IMPORTANT (re-save semantics): record_session_xp_batch does NOT run the
-- BUG-RPG-001 reversal pattern itself. The reversal pattern stays in
-- save_workout because (a) it must run BEFORE the cascade-delete, before
-- any new xp_events are visible, and (b) splitting it across both
-- entry points would create a coordination contract that's easy to break.
--
-- BodyPart enum index contract (must match Dart `BodyPart` enum + spec
-- §5.2 attribution keys):
--   1 = chest      2 = back       3 = legs       4 = shoulders
--   5 = arms       6 = core       7 = cardio
-- An unrecognized body_part key (which would indicate a corrupted
-- xp_attribution map) raises an EXCEPTION rather than silently dropping
-- the contribution — fail loud, fix the data.
...
-- ---------------------------------------------------------------------------
-- 13. save_workout — extend to call record_session_xp_batch once per session
-- ---------------------------------------------------------------------------
--
-- The atomicity guarantee of save_workout is preserved: the batch is called
-- inside the same transaction, so a constraint violation in either the set
-- INSERT or the XP roll-up rolls back the whole save.
--
-- Performance: BUG-RPG-004 fix. The original implementation looped
-- record_set_xp per set, paying ~4 ms warm × N sets in PL/pgSQL function
-- dispatch + parameter binding + per-call session/weekly subqueries. EXPLAIN
-- ANALYZE on a 100-set payload measured ~424 ms (8.5× the 50 ms gate). The
-- batch consolidates that work into a single function call with bulk
-- INSERT/UPSERTs; the same EXPLAIN ANALYZE drops well under 50 ms.
...
BEGIN
  ...
  -- RPG v1 XP roll-up: ONE batch call inside the same transaction, replacing
  -- the per-set record_set_xp loop that was 8× over the 50ms perf gate
  -- (BUG-RPG-004). The batch iterates sets in (we.order, s.set_number)
  -- order — same total ordering the per-set loop used — and writes
  -- xp_events / body_part_progress / exercise_peak_loads in three bulk
  -- statements at the end. Numeric semantics are identical to record_set_xp
  -- (running session_vol / weekly_vol per body part, peak advancement
  -- before strength_mult); the 16 integration tests in test/integration/
  -- gate parity within 0.01.
