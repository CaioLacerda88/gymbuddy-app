...
|---|------|------|------------|----------------|-----------------|--------|
| 1 | `specs/offline-sync.spec.ts` | OFFLINE-001 | pre-Phase-18c | TEST-INFRA | Phase 18c moved the Finish Workout button from the bottom bar to an AppBar trailing OutlinedButton labelled "FINISH". The spec used `page.click('text=Finish Workout')` — a text= selector that no longer matches. Fixed by switching to `WORKOUT.finishButton` (`[flt-semantics-identifier="workout-finish-btn"]`) and `WORKOUT.dialogFinishButton` for the confirm dialog. | DISCHARGED 2026-04-28 — Family 4 fix. 40/40 at `--repeat-each=10 --retries=0`. |
| 2 | `specs/offline-sync.spec.ts` | OFFLINE-002 | pre-Phase-18c | TEST-INFRA | Same root cause as OFFLINE-001. | DISCHARGED 2026-04-28 — Family 4 fix. Same selector update. 40/40 at `--repeat-each=10 --retries=0`. |
| 3 | `specs/offline-sync.spec.ts` | OFFLINE-005 | pre-Phase-18c | TEST-INFRA | Same root cause as OFFLINE-001. | DISCHARGED 2026-04-28 — Family 4 fix. Same selector update. 40/40 at `--repeat-each=10 --retries=0`. |
| 4 | `specs/offline-sync.spec.ts` | OFFLINE-007 | pre-Phase-18c | TEST-INFRA | Same root cause as OFFLINE-001. | DISCHARGED 2026-04-28 — Family 4 fix. Same selector update. 40/40 at `--repeat-each=10 --retries=0`. |
...
| 13 | `specs/workouts.spec.ts` | full-journey smoke | (stale entry) | Original entry described `waitForTimeout` polls in the helper chain that no longer exist — the test now uses `dismissCelebrationIfPresent` (Family 2 helper) and explicit `toBeVisible({ timeout })` assertions. | DISCHARGED 2026-04-28 — Family 6 verification. No code change needed beyond Family 1/2 helper adoption. 20/20 at `--repeat-each=20 --retries=0`. Stage 2 regression also passed 1/1 at `--retries=0`. |
...
| 15 | `specs/workouts.spec.ts` | decimal-weight round-trip (WK-023) | Locale-dependent decimal parsing flake | en/pt locale switching mid-test | DISCHARGED 2026-04-28 — Family 5 fix. Root cause was missing URL-wait after history card tap: `waitForURL(/\/home\/history\//)` now gates the `text=22.5` assertion so Riverpod has had time to restore the detail from Hive cache. 20/20 at `--repeat-each=20 --retries=0`. |
...
| 20 | `specs/exercises-localization.spec.ts` | A1 / A2 / B1 (3 tests) | Locale switch flush timing | Translation cache vs localized name fetch race | DISCHARGED 2026-04-28 — Family 5 fix. Root cause was `waitForTimeout(800)` racing the Supabase `fn_search_exercises_localized` RPC response under CI load. Replaced with `waitForResponse` registered before `flutterFill`. 60/60 at `--repeat-each=20 --retries=0`. |
| 21 | `specs/exercises.spec.ts` | clear-search | Search-input clear race | Flutter web hidden input proxy event ordering | DISCHARGED 2026-04-28 — Family 5 fix. Root cause was `toHaveValue('')` firing before Flutter re-established the text editing connection on screen remount. Added `waitFor({ state: 'attached' })` on the native `<input>` proxy before asserting. 20/20 at `--repeat-each=20 --retries=0`. |
...
4. **offline-sync** (#1–#4) — DISCHARGED 2026-04-28. Root cause was outdated `text=Finish Workout` / `text=Save & Finish` selectors: Phase 18c moved the finish button to the AppBar. Fixed with stable `WORKOUT.finishButton` / `WORKOUT.dialogFinishButton` identifiers. 40/40 at `--repeat-each=10 --retries=0`.
