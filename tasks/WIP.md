...
- [ ] **Step 1 — Build name → folder map.** Fetch `dist/exercises.json` from `yuhonas/free-exercise-db`. Build `{exercise name from migration 00004 → correct folder name in repo}` map. Manually curate any unmatched names. Commit mapping as `tools/exercise_image_mapping.json` (or similar) for audit trail.
- [ ] **Step 2 — Download source images.** For each of the ~57 exercises, fetch `{correct_folder}/0.jpg` and `{correct_folder}/1.jpg` from `raw.githubusercontent.com` into a local staging dir. Validate each file is a non-empty JPEG.
- [ ] **Step 3 — Upload to Supabase Storage.** One-off Dart script `tools/fix_exercise_images.dart` uses `SUPABASE_SERVICE_ROLE_KEY` (read from env, never committed) to push files into `exercise-media/` bucket. Filename convention: `{sanitized_exercise_name}_start.jpg` and `{sanitized_exercise_name}_end.jpg`. Sanitize: lowercase, spaces → underscores, strip non-alphanumeric. Collision check: error out if any target filename already exists.
- [ ] **Step 4 — Write migration.** `supabase/migrations/00018_fix_exercise_images.sql`: `UPDATE exercises SET image_start_url = 'https://{project}.supabase.co/storage/v1/object/public/exercise-media/{slug}_start.jpg', image_end_url = '...' WHERE name = '{name}' AND is_default = true;` for each of the ~57 exercises. Use the project's hosted Supabase URL (read from `reference_supabase_project.md` memory file, or `.env`).
- [ ] **Step 5 — Verify locally.** Run `npx supabase db reset` (applies all migrations including 00018 on local Supabase); run `flutter run -d chrome`; navigate Exercise List → detail for 3-5 seeded exercises → confirm images render. Check browser devtools Network tab — zero 404s on `exercise-media` requests.
- [ ] **Step 6 — Apply migration to hosted.** `npx supabase db push --linked`. Verify in production via curl sampling 5-10 new URLs → all 200.
- [ ] **Step 7 — Add E2E regression test.** New test in existing `test/e2e/specs/exercises.spec.ts` under the regression (no `@smoke`) block: navigate to an exercise detail, assert `img` elements for start/end have `naturalWidth > 0` (not broken). Wires to the `fullExercises` or `fullExDetailSheet` describe block's existing user.
- [ ] **Step 8 — Run verification gate.** `make ci` must pass (format + analyze + gen + test + android-debug-build). E2E locally: `FLUTTER_APP_URL= npx playwright test specs/exercises.spec.ts` must pass. Evidence pasted in PR body.
