...
- [x] qa-engineer: selector impact assessment done; added +5 unit tests (null-weight handling + `ExerciseProgressKey` equality) and +1 widget test (error-state non-crash), plus a new `smokeExerciseProgress` test user + seed fn and a tagged E2E smoke asserting the chart heading + Semantics label on an exercise with ≥2 logged sessions (1062 → 1068).
- [x] reviewer revision (in-cycle, no deferring) — addressed 2 Important + 2 Nit findings:
  - **Important 1 — chart staleness after save:** `ActiveWorkoutNotifier.finishWorkout` now `ref.invalidate`s the `exerciseProgressProvider` family in the `saveWorkout` success path. New unit test `finishWorkout success: invalidates exerciseProgressProvider family so chart refreshes after save` primes the provider, runs a successful finish, asserts `getExerciseHistory` is hit a second time.
  - **Important 2 — per-exercise window state:** replaced app-global `progressTimeWindowProvider` (`StateProvider<TimeWindow>`) with local widget state inside `ProgressChartSection` (now a `ConsumerStatefulWidget`); `didUpdateWidget` resets the window to `last90Days` when `exerciseId` changes so "All time" on one exercise never leaks into another. Provider deleted entirely — no other consumers. New widget test `window defaults back to 90d when host rebuilds with a new exerciseId` proves the reset via a keyed host swap.
  - **Nit 1 — error state copy conflation:** error branch now renders `"Could not load progress"` (still `bodyMedium` @ 0.4 alpha). Updated the existing error-state widget test to assert the new copy AND absence of the zero-data string.
  - **Nit 2 — single-point mostly whitespace:** single-point branch now renders only the subdued `"1 session logged"` copy (no 200dp `LineChart` canvas), matching the zero-data visual language. Existing single-point widget test updated: `LineChart` must NOT render; only copy. Semantics label preserved so a11y still conveys the count.
  - Net test count: 1068 → 1070 (2 new tests added in the fix commit; one widget test modified not added). Analyzer clean, format clean, all tests green.
- [ ] PR.
